...
func TestTemplateServerFails(t *testing.T) {
	...
	// now flush servers
	require.NoError(ardb.Error(cluster.doAt(0, cluster.servers[0], &ardb.StorageCommand{Type: flushAllCommand})))
	require.NoError(ardb.Error(cluster.doAt(1, cluster.servers[1], &ardb.StorageCommand{Type: flushAllCommand})))
